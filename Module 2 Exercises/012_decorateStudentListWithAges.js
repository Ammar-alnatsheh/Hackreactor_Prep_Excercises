...
function assertArraysEqual(actual, expected, testName) {
  ...
  for ( var i = 0; i < expected.length; i++ ) {
    ...
    var isAgeOk = actual[i].age === 10 || actual[i].age === 11 ;

    if ( actual[i].name !== expected[i].name || !isAgeOk ) {
      equal = false;
      break;
    }
...
assertArraysEqual(decorateClassListWithAges(classList), classListWithAges, 'decorateClassListWithAges');
